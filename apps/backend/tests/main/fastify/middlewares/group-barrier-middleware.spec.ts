...
import { User, GroupMember } from '@/domain/entities';
import { UserNotMemberOfAnyGroupBarrierException, UnauthorizedException } from '@/domain/exceptions';
...
describe('groupBarrierMiddleware', () => {
  ...
  const mockDbUser = User.create({
    ...
    name: 'Test User',
    groups: [
      GroupMember.create({ groupId: 'group-1', userId: 'user-1', joinedAt: new Date() }),
      GroupMember.create({ groupId: 'group-2', userId: 'user-1', joinedAt: new Date() }),
    ],
    createdAt: new Date(),
    updatedAt: new Date(),
  }, 'user-1');
  ...
  it('should throw UnauthorizedException if user is not in request', async () => {
    ...
    await expect(groupBarrierMiddleware(request)).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('should throw UserNotMemberOfAnyGroupBarrierException if user has no groups', async () => {
    ...
    await expect(groupBarrierMiddleware(request)).rejects.toThrow(UserNotMemberOfAnyGroupBarrierException);
  });
